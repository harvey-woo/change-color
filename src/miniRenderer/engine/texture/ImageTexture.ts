...
type IMGType = ImageData | HTMLImageElement | HTMLCanvasElement | HTMLVideoElement | ImageBitmap;
class ImageTexture extends Texture {
	...
	private mTexData: ImgTexData | null = null;
	private mTexDatas: ITexData[] | null = null;
	...
	*/
	setDataFromImage(img: IMGType, miplevel = 0, offsetx = 0, offsety = 0, rebuild = false): void {
		if (img && img.width > 0 && img.height > 0) {
			...
			let td = this.mTexData!;
			if (td != null) {
				if (this.mTexData!.miplevel !== miplevel) {
					if (this.mTexDatas == null) {
						// const nullT = null as ImgTexData;
						this.mTexDatas = new Array(16);
						this.mTexDatas[0] = this.mTexData!;
					}
				...
				this.mTexData = ImgTexData.Create();
				td = this.mTexData;
				td.miplevel = miplevel;
				rebuild = true;
	...
	__$updateToGpu(gl: any | null): void {
