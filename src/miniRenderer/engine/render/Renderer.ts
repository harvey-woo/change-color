...
class Renderer {
	...
	initialize(canvas: HTMLCanvasElement): void {
		if (this.mGL == null) {
			...
			if (this.mGL) {
				const gl = this.mGL!;
				gl.webGLVer = this.mGLVer;

				// this.mClearMask = gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT | gl.STENCIL_BUFFER_BIT;
				// eslint-disable-next-line no-bitwise
				this.mClearMask = gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT;
				this.runBegin();
			} else {
				this.mGLVer = 0;
			}
